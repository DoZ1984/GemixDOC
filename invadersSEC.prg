...
BEGIN
	...
	LOOP
		...
		IF (cont==35) signal(pr1,s_kill_tree); pr3=main_space(); END 
...
BEGIN
	...
	x_alien4=60;
	x_alien5=60;
	dir_alien=0; // dirección de los alien
	vel_alien=3; // velocidad de los alien
	...
	LOOP
		...
		IF (vel_space>15) 
			IF (var_vel==4) vel_space-=1; var_vel=0; END 
		ELSE
			IF (var_vel==3) vel_space-=1; var_vel=0; END 
		END
		...
		IF (fase_arcade==0) t_ENTER=1; previa_arcade(); fase_arcade=1; vidas_alien=0; score_aliens=0; END
	
		IF (fase_arcade==2) fase_arcade=3;
			...
			def(331,75,190,0); def(331,75,185,0); def(335,75,180,0); def(331,80,180,0); def(331,85,180,0); def(331,85,184,0); def(331,90,180,0); def(335,96,180,1); def(331,95,185,0); def(331,95,190,0); def(339,80,186,0); def(339,91,186,1);
			def(331,125,190,0); def(331,125,185,0); def(335,125,180,0); def(331,130,180,0); def(331,135,180,0); def(331,135,184,0); def(331,140,180,0); def(335,146,180,1); def(331,145,185,0); def(331,145,190,0); def(339,130,186,0); def(339,141,186,1);
			def(331,175,190,0); def(331,175,185,0); def(335,175,180,0); def(331,180,180,0); def(331,185,180,0); def(331,185,184,0); def(331,190,180,0); def(335,196,180,1); def(331,195,185,0); def(331,195,190,0); def(339,180,186,0); def(339,191,186,1);
			def(331,225,190,0); def(331,225,185,0); def(335,225,180,0); def(331,230,180,0); def(331,235,180,0); def(331,235,184,0); def(331,240,180,0); def(335,246,180,1); def(331,245,185,0); def(331,245,190,0); def(339,230,186,0); def(339,241,186,1);
		...
		SWITCH (nivel)
			...
			CASE 1 AND fase_arcade>1:
				...
				REPEAT
					alien1(x_alien1,120); x_alien1+=15;
				UNTIL (aliens==11);
	
				REPEAT
					alien1(x_alien2,108); x_alien2+=15;
				UNTIL (aliens==22);
	
				REPEAT
					alien2(x_alien3,96); x_alien3+=15;
				UNTIL (aliens==33);
				
				REPEAT  
					alien2(x_alien4,84); x_alien4+=15;
				UNTIL (aliens==44);
	
				REPEAT
					alien3(x_alien5,72); x_alien5+=15;
				UNTIL (aliens==55);
		...
		IF (saliendo_minijuego==1) transicion=1; cont2=1; fase_arcade=999; saliendo_minijuego=100;
		...
		IF (g_o_a==1) game_over_arcade(); g_o_a=2; END
	
		IF (salto_linea==1) cont++; END
		
		IF (cont==2) salto_linea=0; cont=0; END
	
		IF (cambio_dir==1)
			IF (dir_alien==0) dir_alien=1; ELSE dir_alien=0; END
			cambio_dir=0;
		END
	
		IF (cong_aliens==0 AND rand(0,650)>649 AND hayextra==0 AND g_o_a==0 AND fase_arcade==3) alien_extra(); END
	
		IF (aliens==0) vel_space=18; nivel=1; END
	
		IF (vidas_alien==0 AND fase_arcade==3) g_o_a=1; END
	
		IF (shoot_crash>1) shoot_crash=0; END
	
		// SALIMOS DEL MINI-JUEGO
		IF (cont2>0) cont2++; END
		
		IF (cont2==35) signal(pr3,s_kill_tree); cong_aliens=0; aliens=0;  
			pr1=bar(); cont=0; g_o_a=0; fade(brillo,brillo,brillo,2); saliendo_minijuego=0; fase_arcade=0;
		END
...
BEGIN
	...
	LOOP
		...
		IF (cont==48) t6=write(0,160,170,1,"PRESS ENTER TO PLAY"); cont=0; END
	
		IF (keydown(_ENTER) AND t_ENTER==0) t_ENTER=1;
			IF (cont<24) 
				delete_text(t6); delete_text(t1); delete_text(t2); delete_text(t3); delete_text(t4); delete_text(t5); fase_arcade=2; BREAK;
			END
		END
...
BEGIN
	...
	LOOP
		...
		IF (keydown(_space)) graph=327; y=237; ELSE graph=328; y=238; END
...
BEGIN
	...
	LOOP
		...
		IF (cont2==35) signal(pr3,s_kill_tree); volviendo_minijuego=1; aliens=0; pr1=bar(); cont=0; g_o_a=0; fade(brillo,brillo,brillo,2); END
...
BEGIN
	...
	LOOP
		...
		IF (transicion==0 AND death==0)
			...
			IF (keydown(_left)) run++; x-=vel; IF (run<6) vel=3; ELSE vel=4; END END
			IF (keydown(_right)) run++; x+=vel; IF (run<6) vel=3; ELSE vel=4; END END
			
			IF (NOT keydown(_left) AND NOT keydown(_right)) run=0; END
		
			IF (keydown(_space) AND tope_laser==0 AND tope==0) 
				pr2=laser(x,y-5); tope_laser=1; tope=1; 
		...
		IF (collision(TYPE disparo_alien1) OR collision(TYPE disparo_alien2)) death=1; 
			IF (audio==0) sound_play(snd2,fxv,27); END
		END
		
		IF (death==1) cong_aliens=1; cont++; END
		
		IF (cont==1) graph=301; END
		IF (cont==8) graph=302; END
		IF (cont==15) cont=0; cont2++; END
		
		IF (cont2==5) x=63; graph=300; tope_laser=0; cont2=0; cont=0; vidas_alien-=1; death=0; cong_aliens=0; END
		
		IF (x=<63) x=63; END
		IF (x=>257) x=257; END
		
		IF (g_o_a!=0) IF (audio==0) stop_sound(all_sound); END BREAK; END
...
BEGIN
	...
	LOOP
		...
		IF (cong_aliens==0 AND death==0 AND NOT collision(TYPE def) AND NOT collision(TYPE def) AND NOT collision(TYPE def)) y-=7; END

		IF (y<50) tope_laser=0; BREAK; END
		
		IF (death>0) death++; graph=311; END
		
		IF (death==3 OR death==-1) tope_laser=0; BREAK; END
		
		IF (collision(TYPE disparo_alien1) OR collision(TYPE disparo_alien2)) graph=311; death=1; END
...
BEGIN
	...
	LOOP
		...
		IF (collision(TYPE laser)) tope_laser=0; graph++; 
			IF (graph>tope) BREAK; END
		END
		
		IF (collision(TYPE disparo_alien1) OR collision(TYPE disparo_alien2)) graph++; END
...
BEGIN
	...
	LOOP
		...
		IF (salto_linea==1 AND tope_linea==0) y+=12; tope_linea=1; END
		...
		IF (cont=>vel_space)
			...
			IF (dir_alien==0)
				...
				IF (death==0) graph++; x+=vel_alien; cont=0; END
				IF (x=>260) cambio_dir=1; salto_linea=1; END
			END
			
			IF (dir_alien==1)
			
				IF (death==0) graph++; x-=vel_alien; cont=0; END
				IF (x=<60) cambio_dir=1; salto_linea=1; END
		...
		// COLISIONA CON EL LASER DE LA NAVE DEL PROTAGONISTA
		IF (collision(TYPE laser) AND graph!=311) 
			signal(pr2,s_kill); tope_laser=0; death=1; graph=311; aliens--; 
			score_aliens+=10; var_vel++; suma_exp(80,245,10,fnt6,0); suma_gscore(80,235,100,fnt7,0);
		...
		IF (cont_s==0)
			IF (rand(0,200)>199 AND cong_aliens==0) cont_s=1; disparo_fake(x,y+8); END
		END
		
		IF (canshoot==1) disparo_alien1(x,y); cont_s=1; canshoot=0; END
		...
		IF (y==204 AND linea_aliens<8) linea_aliens=8; g_o_a=1; END
...
BEGIN
	...
	LOOP
		...
		IF (collision(TYPE laser) AND graph!=311) 
			signal(pr2,s_kill); tope_laser=0; death=1; graph=311; aliens--; 
			score_aliens+=20; var_vel++; suma_exp(80,245,20,fnt6,0); suma_gscore(80,235,200,fnt7,0);
		...
		IF (canshoot==1) disparo_alien2(x,y); cont_s=1; canshoot=0; END
		...
		IF (y==180 AND linea_aliens<6) linea_aliens=6; END
		IF (y==192 AND linea_aliens<7) linea_aliens=7; END
		IF (y==204 AND linea_aliens<8) linea_aliens=8; g_o_a=1; BREAK; END
...
BEGIN
	...
	LOOP
		...
		IF (collision(TYPE laser) AND graph!=311) 
			signal(pr2,s_kill); tope_laser=0; death=1; graph=311; aliens--; 
			score_aliens+=30; var_vel++; suma_exp(80,245,30,fnt6,0); suma_gscore(80,235,300,fnt7,0);
...
BEGIN
	...
	LOOP
		...
		IF (y>195) father.canshoot=1; BREAK; END
...
BEGIN
	...
	LOOP
		...
		IF (cong_aliens==0) cont++; cont2++; END
		...
		IF (collision(TYPE laser)) death=1; BREAK; END
...
BEGIN
	...
	LOOP
		...
		IF (cong_aliens==0) cont++;
	
			IF (lado_extra==0) IF (death==0) x+=2; cont=0; END ELSE IF (death==0) x-=2; cont=0; END END
		...
		IF (collision(TYPE laser) AND death==0) 
			IF (x<55) x=55; END IF (x>265) x=265; END
			death=1; score_aliens+=premio_extra; suma_exp(x,y,50,fnt6,0); suma_gscore(x,y-10,500,fnt7,0);
			IF (audio==0) stop_sound(all_sound); sound_play(snd2,fxv,100); END
		END
		...
		IF (death==10) graph=22; text=write_int(0,x-8,y-4,0,OFFSET premio_extra); END
